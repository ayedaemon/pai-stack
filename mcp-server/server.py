...
def register_agents_md():
    """Register AGENTS.md as an MCP resource."""
    agents_file = Path("/app/kb/AGENTS.md")
    if not agents_file.exists():
        log.warning("AGENTS.md not found: %s", agents_file)
        return

    @mcp.resource("skill://agents")
    def read_agents() -> str:
        return agents_file.read_text()

    log.info("registered resource: skill://agents")
