...
| Situation | Use |
|---|---|
| User asks about an external API, RFC, or spec | `notebook_ops(action=search, ...)` |
| You found a relevant doc/article during research | `notebook_ops(action=add_source_url, ...)` |
| You want to persist findings beyond this session | `notebook_ops(action=add_note, ...)` |
| You need to know what research already exists | `notebook_ops(action=list_notebooks)` |
