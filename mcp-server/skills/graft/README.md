...
**Graft** is an MCP skill available in the `pai-stack`.

Code intelligence and contextual code search using Graft
...
**Example Prompt:**
> "Use the `graft` skill to help me with..."

## Technical Details
This skill is parsed and served by the `pai-stack` MCP server. The operational logic, ground rules, and system prompt payloads reside in `SKILL.md`, which is mapped to `skill://graft` for headless agents to fetch.
