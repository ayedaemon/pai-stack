---
name: react
description: Quickly read and work with React codebases — detect toolchain, component model, state, routing, data-fetching.
---
# React Project
...
## When to use
- IF the user mentions React, Next.js, `*.jsx`, `*.tsx`, OR asks to read/debug/build a React app, THEN you MUST execute this skill.
- IF `stack-discovery` finds `react` in `package.json:dependencies`, THEN you MUST execute this skill.
- Preconditions: project under `/opt/data/workspace/...` with `package.json` containing `react`
...
## React Patterns & Development Guidelines

When writing, editing, or planning React code, you MUST adhere to the following conventions:

1. **Architecture & State**:
   - Clear separation of Client vs Server components (Next.js). Use `"use client"` only when necessary.
   - Use derived state instead of redundant `useState`. Extract complex business logic into custom hooks.
   - Strongly type all component props using TypeScript.
2. **Anti-Patterns to Avoid**:
   - Mutating state directly (e.g., `state.value = 1`).
   - Prop drilling deeper than 3 levels (prefer Context or Zustand/Redux).
   - Using `useEffect` for data fetching without abort controllers (strongly prefer React Query/SWR or Next.js fetch).

## Verification
- Mentally trace the component lifecycle to ensure no infinite re-renders or missing hook dependency arrays.
- Verify that UI updates don't break strict mode rules.
