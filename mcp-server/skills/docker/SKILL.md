---
name: docker
description: Read, reason, and safely change Docker/Docker Compose projects — understand build, runtime, networking, volumes.
---
# Docker Project
...
## When to use
- IF the user mentions Docker, `Dockerfile`, `docker-compose.yaml`, `container`, `image`, OR asks to containerize/debug/deploy, THEN you MUST execute this skill.
- IF `stack-discovery` finds `Dockerfile*` or `docker-compose*`, THEN you MUST execute this skill.
- Preconditions: project under `/opt/data/workspace/...` with Dockerfiles
...
## Docker Patterns & Development Guidelines

When writing, editing, or planning Docker configurations, you MUST adhere to the following conventions:

1. **Architecture**:
   - Use multi-stage builds to minimize the final image size.
   - Base images should be minimal (e.g., distroless or alpine) unless otherwise required.
   - Explicitly define a non-root `USER` (never run applications as root inside the container).
2. **Safety & Optimization**:
   - Strict use of `.dockerignore` to prevent context bloat and leaking secrets.
3. **Anti-Patterns to Avoid**:
   - Using `:latest` tags for base images (always pin versions).
   - Storing secrets or API keys in `ENV` directives (use runtime secrets).

## Verification
- Run `docker compose config` (literally or mentally) to validate YAML syntax.
- Verify that a non-root user is specified and that `.dockerignore` exists.
