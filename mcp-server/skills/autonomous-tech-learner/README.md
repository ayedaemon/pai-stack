...
3. **Persistence**: Instead of losing what it learned when the context window clears, it permanently stores its synthesized notes and source URLs in the **Open Notebook**.
...
> "I want to start using React Server Components. Please use the `autonomous-tech-learner` skill to research the best practices, gotchas, and how they handle state. Store your findings in the Open Notebook."
...
> "Use the `autonomous-tech-learner` skill to study how SurrealDB handles graph relations versus standard foreign keys. I need to know which one we should use for our next feature."
...
2. **Long-Term Memory**: Because it writes to the Open Notebook, other agents in the `pai-stack` (and future sessions) can instantly query this knowledge without needing to learn it all over again.
