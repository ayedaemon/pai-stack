...
**Skill Template** is an MCP skill available in the `pai-stack`.

Template for creating new skills in the pai-stack.
...
**Example Prompt:**
> "Use the `Skill Template` skill to help me with..."

## Technical Details
This skill is parsed and served by the `pai-stack` MCP server. The operational logic, ground rules, and system prompt payloads reside in `SKILL.md`, which is mapped to `skill://Skill Template` for headless agents to fetch.
