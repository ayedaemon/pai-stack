...
- Auto-use: when `stack-discovery` finds `Dockerfile*` or `docker-compose*` at root or `caddy/` `hermes/` style subdirs

## Inputs
- Required: project root path, target service name if compose has many
- Optional: env overrides, host port expectations, Tailscale/Caddy context
...
3. Analyze Dockerfile:
   - Base `FROM` (distroless vs `python:slim` vs `node:alpine` vs `caddy:builder`) + stage names if multi-stage
...
5. Check pai-stack conventions (if repo is pai-stack itself):
   - Build contexts must be flat (`ansible/roles/pai_stack/tasks/main.yml:217` checks `Dockerfile` at root)
   - `kb/` → `silverbulletKB` seeding is `force: no` — never overwrite user edits
   - Caddy holds `tailscaled.sock:ro` + `get_certificate tailscale` for TLS — don't expose public ports
6. Plan change:
   ...
   - If deployment target is pai-stack Pi, remind `ansible/roles/pai_stack/tasks/main.yml:180` copies build contexts explicitly — new files need a copy entry.
...
- `.dockerignore`
- `ansible/roles/pai_stack/tasks/main.yml` (pai-stack deploy conventions)
- `caddy/Caddyfile` (reverse-proxy pattern)
...
- Never suggest `--privileged` or exposing `0.0.0.0` publicly unless via Caddy/Tailscale.
