...
async def ensure_default_chat_model():
    """Ensure a working default chat model is selected."""
    logger.info("--- Checking Default Chat Model ---")
    defaults = await DefaultModels.get_instance()
    if not defaults.default_chat_model:
        # Prefer codestral, gpt-4o-mini, or any available language model
        preferred = ["codestral-2508", "openai/gpt-4o-mini", "codestral-latest", "mistral-small-2603"]
        for p in preferred:
