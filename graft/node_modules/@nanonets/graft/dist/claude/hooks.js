...
import { classifyToolUse, isMcpToolName, isGraftMcpTool, parseSavings, recordToolUse } from './session-metrics.js';
...
export function underGraft(dir, file) {
 ...
 */
export function promptAskTimeout(dir) {
    const installed = installedHookTimeout(dir, 'UserPromptSubmit');
    if (installed === null)
        return CHILD_TIMEOUT_MS - HOOK_OVERHEAD_MS;
 ...
 */
function hookSettingsFiles(dir) {
...
function hookTimeoutIn(file, event) {
    try {
 ...
 */
function withContextDirArg(dir, args) {
    return process.env.GRAFT_DIR ? [...args, '--dir', resolveContextDir(dir)] : args;
}
function graftJson(dir, args, timeout = CHILD_TIMEOUT_MS) {
    try {
        ...
        const out = execFileSync(process.execPath, [cliPath, ...args], { cwd: dir, encoding: 'utf8', timeout, stdio: ['ignore', 'pipe', 'ignore'] });
...
async function handlePostEdit(input, dir) {
    const file = editedFilePath(input, dir);
    if (!file || underGraft(dir, file))
        return;
 ...
 */
export function lastFileScopeHint(dir, lastFile) {
    if (!lastFile)
        return null;
    try {
        const w = readWiring(dir);
        ...
        if (scopes.length <= 1)
            return null; // single-scope: no hint, no --in
        const matches = (w.nodes ?? []).filter((n) => n.kind === 'file' && (n.path === lastFile || n.path.endsWith(`/${lastFile}`)));
        if (matches.length === 0) {
 ...
 * nothing is written.
 */
function handleToolUse(input, dir) {
    recordToolUse(dir, input?.session_id || 'default', { ...classifyAndScore(input?.tool_name, input?.tool_input?.command, () => input?.tool_response ?? input), host: 'claude-code' });
}
/**
 ...
 */
function classifyAndScore(toolName, command, payload) {
    let kind = classifyToolUse(toolName, command);
    if (kind === 'source')
        return { kind, savedTokens: 0 };
 ...
 */
function handleCursorPostTool(input, dir) {
    const toolName = String(input?.tool_name ?? '');
    if (isMcpToolName(toolName) || isGraftMcpTool(toolName))
        return; // handled by handleCursorMcp
    const command = input?.tool_input?.command ?? input?.tool_input?.cmd;
    recordToolUse(dir, cursorSessionId(input), { ...classifyAndScore(toolName, command, () => input?.tool_output ?? input?.tool_response ?? input), host: 'cursor' });
}
/**
 ...
 */
function handleCursorMcp(input, dir) {
    const toolName = String(input?.tool_name ?? '');
    if (!isGraftMcpTool(toolName))
        return;
 ...
 */
function sampleTurnCost(input, dir) {
    try {
        ...
        if (billing.uuid === s.lastBillingUuid)
            return;
 ...
 */
function countTallyTurn(input, dir) {
    try {
        const id = input?.session_id || 'default';
        const s = readSession(dir, id);
        if (!s.turnUsedGraft)
        ...
        if (hasSavingsTally(turn.text))
            s.reportedTurns = (s.reportedTurns ?? 0) + 1;
...
export async function main(event) {
    ...
    if (event === 'session-start') {
        ...
        try {
            ...
            if (upkeep.length)
                emit('SessionStart', upkeep.join('\n'));
        }
        return;
    }
    if (event === 'post-edit') {
        await handlePostEdit(input, dir);
        return;
    }
    if (event === 'tool-savings') {
        handleToolUse(input, dir);
        return;
    }
    if (event === 'cursor-post-tool') {
        handleCursorPostTool(input, dir);
        return;
    }
    if (event === 'cursor-mcp') {
        handleCursorMcp(input, dir);
        return;
    }
    ...
    if (event === 'cursor-session-end') {
        summarizeSession(dir, cursorSessionId(input), { host: 'cursor' });
        return;
    }
    if (event === 'stop') {
        handleStop(input, dir);
        return;
    }
    if (event === 'post-edit-sync') {
        await handlePostEdit(input, dir);
        handleStop(input, dir);
        return;
    }
