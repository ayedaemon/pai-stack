...
export function hasSavingsTally(text) {
...
function textOf(content) {
    ...
    if (!Array.isArray(content))
        return '';
    return content.map((p) => (p?.type === 'text' ? String(p.text ?? '') : '')).join('\n');
}
/**
...
export function lastAssistantTurn(transcriptPath) {
    ...
    for (let i = entries.length - 1; i >= 0; i--) {
        ...
        if (!text.trim())
            continue;
        if (uuid === null && typeof e?.uuid === 'string')
            uuid = e.uuid;
        parts.unshift(text);
    }
    if (uuid === null || parts.length === 0)
        return null;
 ...
 */
export function lastTurnBilling(transcriptPath) {
    if (typeof transcriptPath !== 'string' || !transcriptPath)
        return null;
    const tail = readTail(transcriptPath);
    if (!tail)
