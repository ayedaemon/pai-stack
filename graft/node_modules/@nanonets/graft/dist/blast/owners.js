 ...
 * cost to graft.
 */
export function ownersFor(root, files, opts = {}) {
    ...
    for (const line of out.split("\n")) {
        ...
        if (commit === null || !wanted.has(line))
            continue;
        if (isBot(commit.name, commit.email))
            continue;
    ...
    const isExcluded = (o, email) => excluded.has(email.toLowerCase()) ||
 ...
 */
export function diffAuthors(root, base) {
 ...
 */
export function attachOwners(root, report, opts = {}) {
    const ranked = new Map();
    const visit = (label, files, weight) => {
        ...
        for (const o of owners) {
            ...
            if (prev.handle === undefined && o.handle !== undefined)
                prev.handle = o.handle;
        }
        return owners;
    };
    ...
    for (const mod of report.modules)
        mod.owners = visit(mod.label, mod.files, AFFECTED_WEIGHT);
...
export function sinceLabel(last, now = Date.now()) {
