 ...
 */
export function moduleIndex(contextDir) {
    ...
    return {
        ...
        labelOf: (path) => shortLabel(claims.get(path)?.label ?? dirLabel(path)),
        conceptOf: (path) => claims.get(path)?.label ?? null,
    };
}
...
export function shortLabel(label) {
    ...
    if (bare.length <= MAX_LABEL)
        return bare;
    const at = CLAUSE_BREAK.exec(bare);
    if (at && at.index >= 8 && at.index <= MAX_LABEL)
        return bare.slice(0, at.index).trimEnd();
