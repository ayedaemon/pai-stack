...
import { MAX_REVIEWERS, mention, sinceLabel } from "./owners.js";
...
const TEST_GLYPH = { changed: "✓", stale: "⚠", none: "✗", na: "–" };
/**
 ...
 */
export function mermaidDiagram(r) {
 ...
 */
export function markdownReport(r, opts = {}) {
    const out = [];
...
function testHeadline(r) {
    ...
    const parts = [];
    if (none.length > 0)
        parts.push(`**no test reaches ${none.map((a) => a.label).join(", ")}**`);
    if (stale.length > 0)
        parts.push(`${stale.map((a) => a.label).join(", ")} ${stale.length === 1 ? "has tests" : "have tests"} the diff did not touch`);
    if (changed.length > 0)
        parts.push(`${plural(changed.length, "area")} updated ${changed.length === 1 ? "its" : "their"} tests`);
    if (parts.length === 0)
        return null;
 ...
 * CODEOWNERS.
 */
function ownerSection(r) {
    if (r.reviewers === undefined)
        return []; // taken outside a git repository
    const rows = [
        ...r.areas.map((a) => ({ label: a.label, side: "changed", owners: a.owners })),
        ...r.modules.map((m) => ({ label: m.label, side: "affected", owners: m.owners })),
    ];
    ...
    for (const row of rows)
        for (const o of row.owners ?? [])
            people.add(o.handle ?? o.name);
    const out = ["<details>"];
    out.push(`<summary><strong>Who knows this code</strong> — ${plural(people.size, "person", "people")} ` +
    ...
    out.push("");
    out.push("_Ownership is git history over each area's own files, weighted towards recent work " +
 ...
 */
function detailSections(r, symbols, evidence) {
    const out = [];
    if (symbols > 0) {
        out.push("<details>");
        ...
        for (const mod of r.modules) {
            ...
            for (const s of mod.symbols) {
                ...
                if (line)
                    out.push(`  \`\`\`${line.n}: ${line.text.trim()}\`\`\``);
            }
            out.push("");
            if (listed >= MAX_SYMBOLS_LISTED) {
    ...
    if (r.areas.length > 0) {
        const by = (s) => r.areas.filter((a) => a.tests === s).length;
        const states = [
        ...
        for (const a of r.areas) {
            ...
            if (a.changedTestFiles.length > 0)
                bits.push(`${plural(a.changedTestFiles.length, "test file")} changed here: ${a.changedTestFiles.map((f) => `\`${f}\``).join(", ")}`);
            else if (a.testFiles.length > 0)
                bits.push(`${plural(a.testFiles.length, "test file")} ${a.testFiles.length === 1 ? "reaches" : "reach"} it, none changed here`);
            else if (a.behavioural > 0)
                bits.push("no test file reaches it");
            else
                bits.push("no function, method or class changed here");
 ...
 * dependents" as "safe".
 */
function caveatLines(r) {
    const out = [];
    if (r.deleted.length > 0) {
        out.push(`⚠️ ${plural(r.deleted.length, "deleted file")} (${r.deleted.slice(0, 5).join(", ")}) — ` +
    ...
    if (r.unindexed.length > 0) {
        out.push(`⚠️ ${plural(r.unindexed.length, "changed file")} not in the graph (${r.unindexed.slice(0, 5).join(", ")}) — ` +
...
export function textReport(r) {
    ...
    for (const mod of r.modules) {
        ...
        if (hidden > 0)
            lines.push(`  …${plural(hidden, "more symbol")}`);
        lines.push("");
    }
    if (symbols === 0)
        lines.push("no indexed dependents outside the changed files themselves", "");
    if (r.testModules.length > 0) {
        const files = new Set(r.testModules.flatMap((m) => m.files));
    ...
    for (const line of caveatLines(r))
        lines.push(line.replace(/⚠️ /, "⚠ "), "");
    return lines.join("\n").replace(/\n+$/, "\n");
}
