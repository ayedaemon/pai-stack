...
export function inputUsdPerMtok(model) {
    if (typeof model !== 'string')
        return null;
    for (const [pattern, usd] of INPUT_USD_PER_MTOK)
 ...
 * have no price for. */
export function turnInputCostMicros(usage) {
    const list = inputUsdPerMtok(usage.model);
    if (list === null)
        return null;
    const weighted = usage.input +
 ...
 * of the blended rate. */
export function turnInputTokens(usage) {
 ...
 */
export function dollarsSaved(savedTokens, costMicros, tokensBilled) {
    if (!costMicros || !tokensBilled || savedTokens <= 0)
        return null;
