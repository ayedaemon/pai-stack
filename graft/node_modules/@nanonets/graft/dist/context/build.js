...
function resolveOnlyDirs(outDir, explicit) {
 ...
 */
export function listContextFiles(root, outDir, exts, explicitOnlyDirs) {
...
export async function buildContext(dir, opts) {
    ...
    const work = await mapWithConcurrency(files, Math.max(1, opts.concurrency ?? 8), async (file, i) => {
        ...
        try {
            ...
            if (decoded === null)
                return undefined; // unsupported encoding (e.g. UTF-16BE) — skip, not an error
            code = decoded;
        }
        catch (err) {
    ...
    saveCache(outDir, cache);
    const processed = work.filter((w) => w !== undefined);
    ...
    cache.synth = Object.fromEntries(batches.flatMap((batch) => {
    ...
    for (const n of synthNodes) {
        ...
        if ((n.summary?.length ?? 0) > draft.summary.length)
            draft.summary = n.summary;
        if (n.type && draft.type === "concept")
            draft.type = n.type;
        for (const src of n.sources) {
            const hash = hashByPath.get(src);
            if (hash)
                draft.sources.set(src, hash);
        }
    ...
    for (const n of synthNodes) {
        ...
        for (const link of n.links) {
            ...
            if (!from.links.has(key))
                from.links.set(key, { to, relation: link.relation, description: link.description });
        }
    }
    ...
    for (const draft of drafts.values()) {
        if (draft.sources.size > 0)
            continue;
        for (const link of draft.links.values()) {
            const target = drafts.get(link.to);
            if (target)
                for (const [p, h] of target.sources)
                    draft.sources.set(p, h);
        }
    }
    // Phase 5: finalize, reconcile with disk, write.
    const nodes = [...drafts.values()].map((d) => {
        const sources = [...d.sources.entries()]
    ...
    if (result.links === 0 && result.batches > 1) {
        console.error(`⚠ synthesis produced 0 links across ${result.batches} batches — the model may be degrading; see per-batch counts above`);
    }
    // Manifest: authoritative file→hash map (every processed file) + node roster.
    const fileRefs = processed
...
function batchKey(batch, hashByPath) {
    return contentHash(batch
        .map((f) => `${f.path}:${hashByPath.get(f.path) ?? ""}`)
        .sort()
        .join("\n"));
}
function registerName(table, name, slug) {
    const key = name.trim().toLowerCase();
    if (key && !table.has(key))
        table.set(key, slug);
}
function resolveSlug(table, name) {
