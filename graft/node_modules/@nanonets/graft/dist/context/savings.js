...
export function toTokens(chars) {
...
function fileSizes(graph) {
    const m = new Map();
    for (const n of graph.nodes)
        if (n.kind === 'file' && typeof n.chars === 'number')
            m.set(n.path, n.chars);
    return m;
}
...
export function savingsFor(graph, paths) {
...
export function setInputRate(usdPerMtok) {
    inputRateUsdPerMtok =
...
export function savingsLine(body, saved) {
    if (!saved || saved.baselineChars <= 0)
        return '';
    ...
    return (`[graft] tokens saved ≈ ${delta.toLocaleString()} (${pct}%) — this output ≈ ` +
 ...
 */
export function sumSavingsFooters(text) {
    let total = 0;
...
export function withSavings(body, saved) {
