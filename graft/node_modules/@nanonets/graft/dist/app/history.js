 ...
 */
export function readCommits(root, max = MAX_COMMITS) {
 ...
 */
export async function readThreads(owner, repo, token, fetchImpl, api = "https://api.github.com", max = MAX_THREADS) {
    const headers = {
    ...
    for (let page = 1; pulls.length < max && page <= Math.ceil(max / 100); page++) {
        const res = await fetchImpl(`${api}/repos/${owner}/${repo}/pulls?state=closed&sort=updated&direction=desc&per_page=100&page=${page}`, { headers });
        if (!res.ok)
            break;
    ...
    const candidates = pulls.filter((p) => typeof p.number === "number").slice(0, max);
    ...
    for (let i = 0; i < candidates.length; i += THREAD_FETCH_CONCURRENCY) {
        ...
        const fetched = await Promise.all(slice.map(async (p) => {
...
async function readComments(url, headers, fetchImpl) {
    const out = [];
    for (let page = 1; page <= MAX_COMMENT_PAGES; page++) {
        ...
        let batch;
        try {
            batch = JSON.parse(await res.text());
        }
        catch {
            break;
        }
 ...
 * likely to govern.
 */
export function readSymbols(graph, max = MAX_SYMBOLS) {
    if (!graph)
        return [];
    return graph.nodes
 ...
 */
export async function postDigest(baseUrl, brainId, brainToken, digest, fetchImpl) {
