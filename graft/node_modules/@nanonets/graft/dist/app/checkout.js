...
function git(dir, args, token) {
    const auth = token
        ...
        : [];
    const res = spawnSync("git", ["-c", "core.hooksPath=/dev/null", "-c", "protocol.version=2", ...auth, ...args], {
        cwd: dir,
        encoding: "utf8",
 ...
 */
export function checkoutPullRequest(req) {
    ...
    if (against === null) {
        fail("diff base", `refs/pull/${req.number}/merge is gone and refs/pull/${req.number}/head gives no diff against ` +
 ...
 */
export function checkoutRepository(req) {
    const host = (req.api ?? "https://github.com").replace(/\/$/, "");
    ...
    const fetched = git(dir, ["fetch", "--quiet", `--depth=${depth}`, "--no-recurse-submodules", "origin", refspec], req.token);
    if (!fetched.ok)
        fail("fetch", fetched.err);
...
function fetchArgs(req, pull, deepen = false) {
    return [
        "fetch",
        "--quiet",
...
function headDiffBase(dir, req) {
    for (const deepen of [false, true]) {
        ...
        if (deepen && !git(dir, fetchArgs(req, "head", true), req.token).ok)
            return null;
...
function resolveHeadBase(dir, req) {
    ...
    if (line(dir, ["merge-base", parent, "refs/graft/head"]) === null)
        return null;
    if (!git(dir, ["update-ref", "refs/graft/base", parent]).ok)
        return null;
