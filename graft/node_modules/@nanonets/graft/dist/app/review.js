...
const DEPTH = 2;
export async function reviewPullRequest(job, deps) {
    const log = deps.log ?? (() => { });
    ...
    try {
        ...
        if (!graph)
            throw new Error("build produced no graph");
        const diff = changedFiles(checkout.dir, checkout.base);
        if (!diff)
            throw new Error(`no diff against ${job.baseRef}`);
        ...
        if (note)
            log(`${job.owner}/${job.repo}#${job.number}: ${note}`);
...
async function exportPage(report, contextDir, root, job) {
 ...
 */
async function upsertComment(job, body, token, deps) {
    ...
    if (!res.ok)
        throw new Error(`comment failed: ${res.status} ${text.slice(0, 200)}`);
    return JSON.parse(text).html_url ?? null;
}
async function findComment(base, headers, fetchImpl) {
