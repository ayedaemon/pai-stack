...
import { appJwt, installationFor, repoAccessGap } from "./identity.js";
import { buildDigest, postDigest, readCommits, readSymbols, readThreads } from "./history.js";
import { budgetSources, readAgentInstructions, readBranchProtection, readCodeowners, readCodifiedRules, readDecisionDocs, readDeclinedIssues, readReverts, readTestNames, } from "./sources.js";
...
export class RepoNotAccessibleError extends Error {
    ...
    gap = { reason: "not_installed", ownerId: null, installationId: null }) {
        super(message);
        this.gap = gap;
    }
}
 ...
 */
export async function buildRepoIntoBrain(job, deps) {
    const log = deps.log ?? (() => { });
    ...
    if (installationId === null) {
        ...
        throw new RepoNotAccessibleError(gap.reason === "repo_not_selected"
            ? `graft is installed on ${job.owner} but ${tag} is not in the list of repositories it can see`
            : `graft is not installed on ${job.owner}`, gap);
    }
    ...
    const token = JSON.parse(await tokenRes.text()).token;
    ...
    try {
        ...
        log(`${tag}: read ${commits.length} commits, ${threads.length} threads, ${symbols.length} symbols, ${sources.length} stated sources`);
        ...
        const { jobId } = await postDigest(job.brainBaseUrl ?? "https://agents.nanonets.com", job.brainId, job.brainToken, digest, deps.fetch);
        return { jobId, ...counts };
    }
    finally {
 ...
 */
async function repoMeta(owner, repo, token, fetchImpl, api) {
    try {
...
//# sourceMappingURL=brain-build.js.map