...
export class PageStore {
    ...
    get(id, token) {
        ...
        if (!page || !token || !this.valid(id, token))
            return null;
    ...
    hydrate() {
        ...
        for (const name of names) {
            ...
            if (page)
                loaded.push([name.slice(0, -SUFFIX.length), page]);
        }
    ...
    read(file) {
        try {
            ...
            const { html, storedMs } = (parsed ?? {});
            if (typeof html !== "string" || typeof storedMs !== "number" || !Number.isFinite(storedMs))
                return null;
