 ...
 */
export function summarizeSession(repo, id, opts = {}) {
    try {
        const { host = 'claude-code', home, env } = opts;
        if (!telemetryOn(home, env))
            return 0;
 ...
 */
export function flushClosedSessions(repo, now = Date.now(), home, env, host = 'claude-code') {
    try {
        if (!telemetryOn(home, env))
            return 0;
        ...
        for (const id of listSessionIds(repo).sort()) {
            ...
            let mtime;
            try {
                mtime = statSync(join(dir, `${id}.json`)).mtimeMs;
            }
            catch {
                continue;
            }
            if (now - mtime < SESSION_IDLE_MS)
                continue;
