...
export function isTrackedCommand(name) {
    return TRACKED_COMMANDS.includes(name);
}
/** Where a failing build died. Coarse on purpose: enough to route a bug, not
 *  enough to describe anyone's repo. */
export const BUILD_STAGES = ['walk', 'extract', 'graph', 'summarize', 'synthesize', 'write'];
/**
...
export function savedTokensBucket(n) {
    if (n <= 0)
        return '0';
    if (n < 1000)
        return '<1k';
 ...
 */
export function langsValue(langs) {
    const clean = langs
