...
function queueHasEvents(home) {
    try {
        return statSync(queuePath(home)).size > 0;
    }
    catch {
        return false;
    }
}
export function needsFlush(flushedAt, now = Date.now()) {
 ...
 */
export function maybeFlushInBackground(home, now = Date.now()) {
    try {
        if (!telemetryOn(home))
            return false;
        if (!queueHasEvents(home))
            return false;
        if (!needsFlush(readState(home)?.flushedAt, now))
            return false;
...
export async function runFlush(home) {
    try {
        ...
        if (events.length === 0)
            return;
        if (shouldRequeue(await sendBatch(events)))
            requeue(events, home);
    }
