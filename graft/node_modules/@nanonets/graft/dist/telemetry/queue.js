 ...
 */
export function enqueue(event, home) {
...
export function drain(home) {
    ...
    for (const line of text.split('\n')) {
        ...
        catch { /* torn line — skip just this one */ }
    }
    return out;
}
/** Read without draining, for `graft telemetry debug`. */
export function peek(home) {
    const out = [];
    let text = '';
    try {
        text = readFileSync(queuePath(home), 'utf8');
    }
    catch {
        return out;
    }
 ...
 *  since the drain. */
export function requeue(events, home) {
    if (events.length === 0)
        return;
    const path = queuePath(home);
    try {
