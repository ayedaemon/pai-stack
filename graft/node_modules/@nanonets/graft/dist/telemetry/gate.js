...
export function doNotTrack(env = process.env) {
...
export function offReason(home, env = process.env) {
    if (!posthogKey())
        return 'no-key';
    if (doNotTrack(env))
        return 'do-not-track';
    if (inCi(env))
        return 'ci';
    if (readState(home)?.enabled === false)
        return 'disabled';
    return null;
}
export function telemetryOn(home, env = process.env) {
...
export function explainOff(reason) {
    switch (reason) {
