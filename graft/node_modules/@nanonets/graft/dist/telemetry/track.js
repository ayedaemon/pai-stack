 ...
 */
export function detectHost(explicit, env = process.env) {
    if (explicit)
        return explicit;
...
function commonProps(ctx) {
    ...
    if (ctx.repo) {
        try {
            props.repo_id = repoId(ctx.repo);
        }
        catch { /* unwritable cache — omit */ }
    }
    return props;
}
/**
 ...
 */
export function track(event, props = {}, ctx = {}) {
    try {
        if (!telemetryOn(ctx.home, ctx.env))
            return null;
 ...
 */
export function trackFirstRunIfNew(ctx = {}) {
    try {
        if (!telemetryOn(ctx.home, ctx.env))
            return;
        if (readState(ctx.home)?.firstRunAt)
            return;
        track('first_run', {}, ctx);
 ...
 */
export function trackInstallIfNew(ctx = {}) {
    try {
        if (!telemetryOn(ctx.home, ctx.env))
            return;
        const version = runningVersion();
        if (readState(ctx.home)?.installedVersion === version)
            return;
