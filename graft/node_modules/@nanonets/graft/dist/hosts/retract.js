...
import { ALL_MARKERS } from './sections.js';
...
function removeFile(path, apply) {
    if (!existsSync(path))
        return 'absent';
    if (apply) {
 ...
 */
function stripSection(path, apply, markers = ALL_MARKERS) {
    if (!existsSync(path))
        return 'absent';
    ...
    if (!markers.some((m) => text.includes(m.start)))
        return 'absent';
    ...
    const collapsed = out
        ...
        .replace(/\n+$/, '\n');
    if (!apply)
        return isBlank(collapsed) ? 'deleted' : 'removed';
 ...
 */
function removeJsonKey(path, topKey, apply) {
    if (!existsSync(path))
        return 'absent';
    let root;
    try {
 ...
 */
function removeTomlSection(path, apply) {
    if (!existsSync(path))
        return 'absent';
    ...
    if (!found)
        return 'absent';
    if (!apply)
        return isBlank(kept) ? 'deleted' : 'removed';
    if (isBlank(kept))
        return removeFile(path, true);
 ...
 */
function stripClaudeSettings(path, apply) {
    if (!existsSync(path))
        return 'absent';
    let root;
    try {
    ...
    const isGraftCmd = (v) => JSON.stringify(v ?? '').includes('graft-statusline.cjs');
    for (const key of ['statusLine', 'subagentStatusLine']) {
        if (root[key] !== undefined && isGraftCmd(root[key]))
            delete root[key];
    }
    if (root.hooks && typeof root.hooks === 'object' && !Array.isArray(root.hooks)) {
        for (const event of Object.keys(root.hooks)) {
            ...
            if (!Array.isArray(prior))
                continue;
            const kept = prior.filter((e) => !JSON.stringify(e ?? '').includes('graft-hooks.cjs'));
            if (kept.length === 0)
    ...
    if (Array.isArray(root.footerLinksRegexes)) {
        const kept = root.footerLinksRegexes.filter((r) => !isGraftFooterRegex(r));
        if (kept.length === 0)
            delete root.footerLinksRegexes;
        else
            root.footerLinksRegexes = kept;
    }
    if (root.permissions && Array.isArray(root.permissions.allow)) {
        const kept = root.permissions.allow.filter((a) => !isGraftAllowEntry(a));
        if (kept.length === 0)
            delete root.permissions.allow;
        else
            root.permissions.allow = kept;
        if (Object.keys(root.permissions).length === 0)
    ...
    if (after === before)
        return 'absent';
    if (!apply)
        return Object.keys(root).length === 0 ? 'deleted' : 'removed';
    if (Object.keys(root).length === 0)
        return removeFile(path, true);
    writeFileSync(path, `${JSON.stringify(root, null, 2)}\n`);
    return 'removed';
...
function stripCodexHooks(path, apply) {
    if (!existsSync(path))
        return 'absent';
    let root;
    try {
 ...
 */
function stripIgnoreEntries(path, entries, apply) {
    if (!existsSync(path))
        return 'absent';
    const text = readFileSync(path, 'utf8');
    const kept = text
        ...
        .filter((l) => {
        const t = l.trim();
        if (t.startsWith('#') && t.includes('graft'))
            return false;
...
function targets(repo, opts) {
    ...
    for (const host of HOSTS) {
        if (exclude.has(host.id))
            keptPaths.add(join(repo, host.relPath));
    }
    for (const t of mcpTargets(repo, [...exclude], { home }))
        keptPaths.add(t.path);
    if (exclude.has('claude')) {
        for (const t of claudeTargets(repo))
            keptPaths.add(t.path);
        for (const t of claudeGlobalTargets(home))
            keptPaths.add(t.path);
    }
    ...
    const seen = new Set();
    const add = (t) => {
        if (keptPaths.has(t.path) || seen.has(t.path))
            return;
        seen.add(t.path);
        out.push(t);
    };
    ...
    for (const t of mcpTargets(repo, allIds, { home })) {
        if (opts.global === false && t.scope === 'global')
            continue;
        add({
...
export function planRetract(repo, opts = {}) {
 ...
 */
export function runRetract(repo, opts = {}) {
...
export function changed(retractions) {
