...
function resolveCommand(cmd) {
    ...
    let abs = null;
    try {
        abs = execSync(`command -v ${cmd}`, { encoding: "utf8" }).trim() || null;
    }
    catch {
        abs = null;
    }
...
export function pickServer(languagesPresent) {
    for (const s of LSP_SERVERS) {
        if (!s.languages.some((l) => languagesPresent.has(l)))
            continue;
        const abs = resolveCommand(s.command);
        if (abs)
            return { ...s, command: abs };
    }
    return null;
}
