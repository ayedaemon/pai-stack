...
const DEFN = new Set(["function", "method", "class", "struct", "interface", "type", "enum"]);
const langOf = (path) => genericLangOf(path)?.name ?? languageLabelOf(path);
const parseSpan = (s) => {
    const m = /^L(\d+)-L(\d+)$/.exec(s);
    return m ? [Number(m[1]), Number(m[2])] : null;
};
export async function enrichWithLsp(graph, root, opts = {}) {
    const languagesPresent = new Set();
    for (const n of graph.nodes) {
        const l = langOf(n.path);
        if (l)
            languagesPresent.add(l);
    }
    const server = pickServer(languagesPresent);
    if (!server)
        return { added: 0, queried: 0, server: null };
    ...
    for (const n of graph.nodes) {
        if (n.kind === "file" || !DEFN.has(n.kind))
            continue;
        const sp = parseSpan(n.span);
        if (!sp)
            continue;
    ...
    if (opts.maxNodes && sources.length > opts.maxNodes)
        sources = sources.slice(0, opts.maxNodes);
    ...
    const linesOf = (rel) => {
        if (!fileLines.has(rel)) {
            try {
                fileLines.set(rel, readFileSync(join(root, rel), "utf8").split("\n"));
            }
            catch {
                fileLines.set(rel, []);
            }
    ...
    for (const src of sources) {
        ...
        for (const callee of callees) {
            let calleeAbs;
            try {
                calleeAbs = fileURLToPath(callee.uri);
            }
            catch {
                continue;
            }
            ...
            if (existing.has(key))
                continue;
            existing.add(key);
            graph.edges.push({ source: src.id, target: target.id, relation: "calls", confidence: "lsp_resolved" });
            added++;
        }
    }
    await client.dispose();
