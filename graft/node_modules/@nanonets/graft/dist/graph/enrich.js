 ...
 */
import { formatCruxMiss } from "../ai/crux.js";
...
export async function enrichGraph(nodes, prior, sources, opts = {}) {
    const stats = {
        cached: 0,
        computed: 0,
        stale: 0,
...
function cruxMissMessage(summarizer, fallback) {
 ...
 */
async function collectFileCrux(summarizer, path, source, refs) {
 ...
 */
function buildCrux(r, node, source, lineCount) {
    // 0/0 (or an invalid range) is the model saying "nothing to highlight here".
    if (r.crux_start < 1 || r.crux_end < r.crux_start)
        return null;
    ...
    if (end - start + 1 > MAX_CRUX_LINES)
        end = start + MAX_CRUX_LINES - 1;
...
function spanLines(span, fileLines) {
