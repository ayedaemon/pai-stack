 ...
 */
import { extractFile, mintId } from "./extract.js";
import { loadWasmLanguage, parseWasm } from "./generic.js";
...
export function containerExtensions() {
...
export async function warmContainerGrammars(langNames) {
...
function containerFileNode(rel, source, residual) {
    return {
        id: rel,
...
function blocks(root, lang) {
    const out = [];
    const n = root.namedChildCount ?? 0;
    for (let i = 0; i < n; i++) {
        const child = root.namedChild?.(i);
        if (!child || child.type !== lang.block)
            continue;
 ...
 */
export function extractContainer(rel, source, lang) {
    const nodes = [];
    const rawEdges = [];
    const residuals = [];
    ...
    if (root) {
        ...
        for (const body of blocks(root, lang)) {
            ...
            if (scriptFile?.body_text)
                residuals.push(scriptFile.body_text);
