...
function applyMinSubstanceGuard(scopes, nodes) {
    if (scopes.length <= 1)
        return scopes;
    const counts = new Map();
    for (const scope of scopes)
        counts.set(scope.prefix, 0);
...
export async function buildGraph(dir, opts = {}) {
    ...
    const seed = opts.reuse === false ? { seeded: false } : seedGraph(root, { contextDir: opts.contextDir });
    ...
    await warmGenericGrammars(new Set(files.map((f) => genericLangOf(f.abs)?.name).filter((n) => !!n)));
    ...
    await warmContainerGrammars(new Set(files.map((f) => containerLangOf(f.abs)?.name).filter((n) => !!n)));
    files.forEach((f, i) => {
        const rel = f.rel;
    ...
    let cardStats = { written: 0, pruned: 0, files: [] };
    if (!opts.graphOnly) {
