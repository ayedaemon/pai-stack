...
export function resolveContextDir(projectDir) {
    ...
    if (!override)
        return join(projectDir, 'graft');
    return isAbsolute(override) ? override : join(projectDir, override);
}
export function cacheDir(projectDir) { return join(resolveContextDir(projectDir), '.cache'); }
function statsPath(d) { return join(cacheDir(d), 'stats.json'); }
 ...
 */
export function writeJsonAtomic(p, value, compact = false) {
...
export function readStats(d) { return readJson(statsPath(d)); }
export function writeStats(d, s) { writeJsonAtomic(statsPath(d), s); }
...
export function buildConfigPath(d) { return join(d, BUILD_CONFIG_DIR, 'config.json'); }
...
function ensureBuildConfigIgnored(d) {
...
export function readBuildConfig(d) { return readJson(buildConfigPath(d)); }
export function writeBuildConfig(d, c) {
...
export function patchBuildConfig(d, patch) {
...
export function readIncludeDirs(d) {
...
export function readFollowSubmodules(d) {
    return readBuildConfig(d)?.followSubmodules === true;
}
/** Missing and explicit false both retain the backwards-compatible default. */
export function readFollowNestedRepos(d) {
...
export function patchStats(d, patch) {
    const next = { ...(readStats(d) ?? emptyStats()), ...patch };
    writeStats(d, next);
    return next;
}
export function acquireLock(d) {
 ...
 */
export function acquireLockIn(cache) {
    ...
    try {
        ...
        let stale;
        try {
            stale = Date.now() - statSync(p).mtimeMs >= LOCK_STALE_MS;
        }
        catch {
            stale = true;
        }
        ...
        catch { /* another process reclaimed it */ }
        try {
            writeFileSync(p, payload, { flag: 'wx' });
            return true;
        }
        catch (e2) {
            if (e2?.code === 'EEXIST')
...
export function releaseLockIn(cache) {
    try {
        rmSync(join(cache, LOCK_FILE));
    }
    catch { /* already gone */ }
}
