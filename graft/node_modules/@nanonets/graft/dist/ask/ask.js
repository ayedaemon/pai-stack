...
import { withSavings, savingsFor, savingsTurnNudge } from "../context/savings.js";
import { loadGraphCached, loadAskIndexCached } from "../graph/load.js";
import { assertPrefixIndexed, pathUnderPrefix, scopeLabel, scopeOf, scopesHereClause, scopesOfGraph, } from "../graph/scopes.js";
import { normalizePathPrefix } from "../util/paths.js";
import { resolveSymbol } from "../graph/traverse.js";
import { combineComparableScopes, rankScopesAndFuse, } from "./fuse.js";
import { rankFilesBounded, } from "./file-rank.js";
import { fileFirstRoundRobin, roundRobinQueues } from "./file-selection.js";
import { personalizedPageRank, personalizedPageRankPrepared, preparePageRankPartitions, } from "./graphrank.js";
import { readSourceFile } from "../util/source.js";
import { counts, tokenize } from "./index-file.js";
import { rulesForPointers, formatRules } from "../brain/attach.js";
...
function computeIdfFromIndex(index, conceptBags) {
...
function matchedLexicalTerms(query, name, path, body) {
    const terms = new Set();
    for (const term of query.keys())
        if (name.has(term) || path.has(term) || body.has(term))
            terms.add(term);
    return terms;
}
 ...
 * the call site. */
function matchedStrongTerms(query, name) {
    const terms = new Set();
    for (const term of query.keys())
        if (hasTerm(name, term))
            terms.add(term);
    return terms;
}
// ── Structural intent ──────────────────────────────────────────────────────
...
const INCOMING_RELS = ["calls", "references", "implements", "extends"];
const OUTGOING_RELS = ["calls", "references", "imports", "implements", "extends"];
...
function findSubjectNodes(query, graph, inPrefix) {
    const words = subjectWords(query).sort((a, b) => b.length - a.length);
    for (const w of words) {
        let nodes = resolveSymbol(graph, w);
        if (inPrefix)
            nodes = nodes.filter((n) => pathUnderPrefix(n.path, inPrefix));
        if (nodes.length > 0)
            return { nodes, tried: w };
    }
    return { nodes: [], tried: words[0] ?? query };
}
function fallthroughNoteFor(subject) {
    return (`structural index: no entries for '${subject}' — showing lexical matches; ` +
...
function structural(query, graph, limit, inPrefix) {
    ...
    if (subjects.length === 0)
        return { fallthroughNote: fallthroughNoteFor(tried) };
    ...
    for (const e of graph.edges) {
        ...
        if (!ids.has(anchor) || seen.has(other + e.relation))
            continue;
    ...
    if (hits.length === 0)
        return { fallthroughNote: fallthroughNoteFor(subjects[0].name) };
...
export function strongShare(q, node, doc, idf, dfltIdf) {
    if (node.kind === "file")
        return 0;
    return matchedIdfShare(q, [doc.name], idf, dfltIdf);
}
function matchedIdfShare(q, fields, idf, dfltIdf) {
    let matched = 0;
    let total = 0;
    for (const t of q.keys()) {
        const w = idf.get(t) ?? dfltIdf;
        total += w;
        if (fields.some((f) => hasTerm(f, t)))
            matched += w;
    }
...
function normalizedQueryWeights(q, idf, dfltIdf) {
    let total = 0;
    for (const t of q.keys())
        total += idf.get(t) ?? dfltIdf;
    if (total <= 0)
        return new Map();
    return new Map([...q.keys()].map((term) => [term, (idf.get(term) ?? dfltIdf) / total]));
}
function lexical(query, corpus, limit, graphRank, inPrefix, fileFirst = true, fileComplement = false, fileTopLock = false, includeRankingMetadata = false) {
    ...
    const testFactor = (path) => (!wantsTests && isTestPath(path) ? TEST_RANK_PENALTY : 1);
    ...
    if (corpus.askIndex && graph && corpus.askIndex.docs.length === graph.nodes.length) {
        ...
        if (graph.nodes.every((n) => map.has(n.id)))
            docById = map;
    }
    ...
    const symbolDocs = graphNodes.map((n) => {
        ...
        if (d)
            return { n, name: new Map(d.name), path: new Map(d.path), body: new Map(d.body) };
        return {
            n,
    ...
    const selectionGroupOf = new Map();
    ...
    const symbolHits = [];
    const baselineSymbolHits = [];
    const baselineHitById = new Map();
    const fileQueueHitsByGroup = new Map();
    const baselineQueueHitsByGroup = new Map();
    const fileGroups = [];
    const needsFileQueues = fileComplement && (fileTopLock || includeRankingMetadata);
    const makeSymbolHit = (id, hitScore, scope) => {
        const n = byId.get(id);
        if (!n)
            return null;
        const hit = {
    ...
    const rawLexicalById = new Map();
    const matchedTermsById = new Map();
    const matchedStrongTermsById = new Map();
    ...
    if (scopes && scopes.length > 1) {
        ...
        let collapsedComponentById;
        let collapsedFileByRepresentative;
        const fusion = rankScopesAndFuse([...byScope.keys()].sort(), {
            lex: (s) => {
                ...
                for (const { n, name, path, body } of docs) {
                    ...
                    const total = (score(q, name, idf) * 3 +
                        score(q, path, idf) * 2 +
                    ...
                    if (total > 0) {
                        ...
                        if (fileComplement) {
                            ...
                            matchedTermsById.set(n.id, matchedLexicalTerms(q, name, path, body));
                            if (n.kind !== "file")
            ...
            hasIdentifierMatch: (s) => (byScope.get(s) ?? []).some(({ name, path }) => {
                for (const t of q.keys())
                    if (hasTerm(name, t) || hasTerm(path, t))
                        return true;
                return false;
            }),
            walk: (s, seeds) => {
            ...
            collapseCandidates: fileComplement
                ? (candidates) => {
                    ...
                    collapsedComponentById = new Map(candidates.map((candidate) => [candidate.id, candidate]));
                    const fileCandidates = candidates.map((candidate) => {
                        ...
                        return {
                            ...
                            matchedTerms: matchedTermsById.get(candidate.id) ?? new Set(),
                            matchedStrongTerms: matchedStrongTermsById.get(candidate.id) ?? new Set(),
                            eligible: Boolean(n && n.kind !== "file" && rawLexical > 0),
                            spanStart: n ? spanStart(n.span) : undefined,
                        };
                    });
                    const rankedFiles = rankFilesBounded(fileCandidates, fileQueryWeights);
                    collapsedFileByRepresentative = new Map(rankedFiles.map((file) => [file.representative.id, file]));
                ...
                : undefined,
        }, GRAPH_WEIGHT, RESCUE_FLOOR);
        if (needsFileQueues) {
            const baselineFusion = combineComparableScopes(collapsedComponents.map(({ id, scope, score }) => ({ id, scope, score })));
            const baselineScoreById = new Map(baselineFusion.ranked.map((ranked) => [ranked.id, ranked.score]));
            const baselineDisplayOrder = (a, b) => b.score - a.score || symbolTitle(a.id).localeCompare(symbolTitle(b.id));
            ...
            for (const ranked of fusion.ranked) {
                ...
                if (!file)
                    continue;
                const materializeFileQueue = () => file.queue.flatMap((member, index) => {
                    ...
                    const hit = makeSymbolHit(member.id, index === 0
                        ? ranked.score
                        : baselineScoreById.get(member.id) ?? member.baselineScore, component?.scope ?? ranked.scope);
                    return hit ? [hit] : [];
                });
                const materializeBaselineQueue = () => file.queue.flatMap((member) => {
                    const hit = makeSymbolHit(member.id, baselineScoreById.get(member.id) ?? member.baselineScore, collapsedComponentById?.get(member.id)?.scope ?? ranked.scope);
                    return hit ? [hit] : [];
                });
                const hits = includeRankingMetadata
                    ...
                    : (() => {
                        ...
                        if (!leader)
                            return [];
                        const hit = makeSymbolHit(leader.id, ranked.score, collapsedComponentById?.get(leader.id)?.scope ?? ranked.scope);
                        return hit ? [hit] : [];
                    })();
                if (hits.length === 0)
                    continue;
        ...
        const lex = new Map(); // node id → lexical score (>0 only)
        let maxLex = 0;
        ...
        const rankedFiles = fileComplement
            ? rankFilesBounded(singleCandidates.map((candidate) => {
                const rawLexical = rawLexicalById.get(candidate.id) ?? 0;
                return {
                    id: candidate.id,
        ...
        if (needsFileQueues && rankedFiles) {
            const baselineCandidateOrder = (a, b) => b.baseline - a.baseline ||
            ...
            for (const file of rankedFiles) {
                const materializeFileQueue = () => file.queue.flatMap((member, index) => {
                    ...
                    const hit = makeSymbolHit(member.id, index === 0 ? file.score : member.baselineScore);
                    return hit ? [hit] : [];
                });
                const materializeBaselineQueue = () => file.queue.flatMap((member) => {
    ...
    for (const h of conceptHits)
        h.score = maxConcept > 0 ? h.score / maxConcept : 0;
    const scored = [...conceptHits, ...symbolHits];
    scored.sort((a, b) => b.score - a.score || a.title.localeCompare(b.title));
    const baselineScored = needsFileQueues
        ? [...conceptHits, ...baselineSymbolHits].sort((a, b) => b.score - a.score || a.title.localeCompare(b.title))
        : scored;
    const conceptGroups = conceptHits.map((hit, index) => ({
        key: selectionGroupOf.get(hit) ?? `concept:${index}`,
        hits: [hit],
        coverage: matchedOf.get(hit) ?? 0,
        coverageStrong: matchedStrongOf.get(hit) ?? 0,
    }));
    const unlockedGroups = [...conceptGroups, ...fileGroups].sort((a, b) => b.hits[0].score - a.hits[0].score ||
        a.hits[0].title.localeCompare(b.hits[0].title));
    const sameHit = (a, b) => a.kind === b.kind && a.pointer === b.pointer && a.title === b.title;
    ...
    if (fileTopLock && fileFirst && baselineScored[0]) {
        ...
        const baselineQueue = includeRankingMetadata || limit > unlockedGroups.length
            ? baselineQueueHitsByGroup.get(key)?.() ?? baselineScored.filter((hit) => (selectionGroupOf.get(hit) ?? "") === key)
            : [];
    ...
    const selected = fileTopLock && fileFirst
        ...
        : fileFirst
            ? fileFirstRoundRobin(scored.map((hit, index) => ({
...
function sliceSpan(root, path, from, to) {
    try {
        ...
        if (source === null)
            return null; // unsupported encoding (e.g. UTF-16BE)
...
function inlineSource(root, hits, graph, full) {
    const cruxByPointer = new Map();
    if (!full && graph) {
        for (const n of graph.nodes)
            if (n.crux?.code)
                cruxByPointer.set(`${n.path}:${n.span}`, n.crux.code);
    }
    for (const h of hits) {
...
export function ask(dir, query, opts = {}) {
    ...
    if (inPrefix && corpus.graph)
        assertPrefixIndexed(corpus.graph, inPrefix);
    let result;
    if (corpus.graph) {
        ...
        if (outcome && "result" in outcome) {
            ...
            result = lexical(query, corpus, limit, graphRank, inPrefix, fileFirst, fileComplement, fileTopLock, opts.includeRankingMetadata ?? false);
    ...
    if (opts.source) {
        ...
        if (result.ranking) {
            ...
            inlineSource(root, [...new Set(internalHits)], corpus.graph, opts.full ?? false);
        }
...
function attachBrainRules(root, hits, graph) {
    try {
        if (!readLink(root))
            return [];
...
export function skeleton(dir, file, opts = {}) {
    const outDir = contextDirFor(resolve(dir), opts.contextDir);
    const graph = loadGraphCached(outDir);
    if (!graph)
        return { file, entries: [], note: "no wiring graph — run `graft build` first" };
    let defs = graph.nodes.filter((n) => n.kind !== "file" && n.path === file);
    if (!defs.length) {
        const matches = new Set(graph.nodes.filter((n) => n.path === file || n.path.endsWith(`/${file}`)).map((n) => n.path));
        if (matches.size > 1)
            return { file, entries: [], note: `ambiguous — matches: ${[...matches].sort().join(", ")}` };
        const [path] = matches;
        if (path)
            defs = graph.nodes.filter((n) => n.kind !== "file" && n.path === path);
    }
    if (!defs.length)
        return { file, entries: [], note: "no definitions indexed for this file" };
    const startLine = (span) => Number(span.match(/^L(\d+)/)?.[1] ?? 0);
...
export function formatSkeleton(r) {
...
export function formatAsk(r) {
    ...
    if (r.mode === "structural") {
        ...
        r.hits.forEach((h, i) => {
            ...
            if (h.related?.length)
                lines.push(`   related: ${h.related.join(", ")}`);
            if (h.code)
                lines.push("", "```", h.code, "```");
            lines.push("");
        });
    ...
    if (r.rules?.length)
        lines.push(...formatRules(r.rules));
...
function escalationNudge(r) {
    if ((r.mode !== "lexical" && r.mode !== "empty") || r.hits.length > 3)
        return "";
...
function askSavingsLine(r, body) {
    if (!r.saved || r.saved.baselineChars <= 0)
        return "";
    const pack = toTokens(body.length);
    const base = toTokens(r.saved.baselineChars);
    if (base <= pack)
        return ""; // no saving to claim (tiny files); stay quiet
    const saved = base - pack;
    const pct = Math.round((saved / base) * 100);
    return (`[graft] tokens saved ≈ ${saved.toLocaleString()} (${pct}%) — this pack ≈ ` +
...
function scopeFooterLines(r) {
    if (!r.scopes)
        return [];
    const perScope = new Map();
    for (const h of r.hits)
        if (h.scope !== undefined)
            perScope.set(h.scope, (perScope.get(h.scope) ?? 0) + 1);
