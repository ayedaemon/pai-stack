 ...
 */
export function fuseScopes(docs) {
    ...
    if (byScope.size === 1) {
        const [scope, list] = byScope.entries().next().value;
        return {
    ...
    const scopesByBest = [...byScope.entries()].sort((a, b) => b[1][0].score - a[1][0].score || a[0].localeCompare(b[0]));
    ...
    for (const [scope, list] of scopesByBest) {
        if (list[0].score >= gate)
            ...
            alsoMatched.push({ scope, bestId: list[0].id });
    }
    ...
    for (const scope of federated) {
        byScope.get(scope).forEach((d, i) => {
            ...
            if (!prev)
                acc.set(d.id, { scope, score: contribution, bestRank: i });
            else {
    ...
    ranked.sort((a, b) => b.score - a.score || a.scope.localeCompare(b.scope) || a.id.localeCompare(b.id));
    return { ranked, federated, alsoMatched };
}
/**
 ...
 * {@link fuseScopes}.
 */
export function combineComparableScopes(docs) {
    const byScope = new Map();
    for (const d of docs) {
        if (d.score <= 0)
            continue;
    ...
    for (const scope of federated) {
        for (const d of byScope.get(scope)) {
            ...
            if (!prev || d.score > prev.score)
                acc.set(d.id, { scope, score: d.score });
        }
    }
    let max = 0;
    for (const e of acc.values())
 ...
 */
export function rankScopesAndFuse(scopes, ops, graphWeight, rescueFloor) {
    // Pass 1: lexical scoring only, per scope.
    const lexByScope = new Map();
    ...
    if (ops.hasIdentifierMatch) {
        const named = new Map();
        for (const scope of meta.keys())
            named.set(scope, ops.hasIdentifierMatch(scope));
        if ([...named.values()].some(Boolean)) {
            for (const [scope, ok] of named)
                if (!ok)
                    bodyOnly.add(scope);
        }
    }
    ...
    globalMaxLex = 0;
    for (const { maxLex } of meta.values())
        if (maxLex > globalMaxLex)
            globalMaxLex = maxLex;
    ...
    const scoped = ops.collapseCandidates
