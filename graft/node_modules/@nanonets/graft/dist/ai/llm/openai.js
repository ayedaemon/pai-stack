...
function toChatMessage(m) {
    switch (m.role) {
        case "system":
            return { role: "system", content: textPart(m.content, m.cacheBreakpoint) };
        case "user":
            return { role: "user", content: textPart(m.content, m.cacheBreakpoint) };
        case "tool":
            return {
                role: "tool",
        ...
        case "assistant": {
            if (m.providerRaw?.provider === PROVIDER)
                return m.providerRaw.raw;
 ...
 */
function isRejectedObjectToolChoice(err) {
    return (err instanceof OpenAI.APIError &&
        err.status === 400 &&
        /tool_choice/i.test(String(err.message ?? "")));
}
/**
 ...
 */
function isRejectedMaxTokens(err) {
    return (err instanceof OpenAI.APIError &&
        err.status === 400 &&
        /max_tokens.*not supported.*max_completion_tokens/i.test(String(err.message ?? "")));
}
/** Same reasoning-family models: temperature is fixed at 1, not caller-settable. */
function isRejectedTemperature(err) {
    return (err instanceof OpenAI.APIError &&
        err.status === 400 &&
        /temperature.*does not support/i.test(String(err.message ?? "")));
}
/**
 ...
 */
function isRejectedToolsWithReasoning(err) {
...
export class OpenAIChatModel {
        ...
        for (let i = 0; i < 4; i++) {
            try {
                ...
                if (isRejectedToolsWithReasoning(err) && attempt.reasoning_effort === undefined) {
                    attempt = { ...attempt, reasoning_effort: "none" };
                    continue;
                }
    ...
    fromResponse(resp, format) {
        ...
        if (format === "json") {
            ...
            if (jsonCall)
                text = JSON.stringify(jsonCall.args);
