...
import { clearLink } from "./brain/link.js";
...
import { runWorkspaceAsk, runWorkspaceBuild, runWorkspaceCallers, runWorkspaceCheck, runWorkspaceGrep, runWorkspaceMap, } from "./graph/workspace-cli.js";
import { formatInitEpilogue } from "./cli-epilogue.js";
import { planInit, selectedWrites } from "./hosts/plan.js";
import { planRetract, runRetract, changed } from "./hosts/retract.js";
...
import { patchBuildConfig } from "./util/state.js";
...
import { errorCode, filesBucket, durationBucket, formatDebug, formatStatus, firstRunNotice, isTrackedCommand, langsValue, offReason, maybeFlushInBackground, patchState, runFlush, track, trackFirstRunIfNew, } from "./telemetry/index.js";
...
const engineFrom = () => new Graft(cliConfig());
/**
 ...
 */
function warnUnsupportedExtensions(exts) {
    if (!exts?.length)
        return;
...
const DIR_ARG = ["[dir]", "repository root (default: nearest ancestor with a graft/ index)"];
/**
 ...
 */
function queryRoot(dir) {
    if (dir !== undefined)
        return resolve(dir);
    const { root, levels } = nearestGraftRoot(process.cwd(), program.opts().dir);
    if (levels > 0)
        console.error(`[graft] no graft/ here — answering from ${root}/graft`);
    return root;
}
/**
 ...
 */
async function refreshBefore(dir, opts) {
    const globalDir = program.opts().dir;
...
const NO_REFRESH_FLAG = ["--no-refresh", "skip the freshness check — answer from the graph as-is"];
const VIZ_TABS = ["context", "code", "outline"];
/**
...
function parseTabs(raw) {
    ...
    const bad = want.filter((t) => !VIZ_TABS.includes(t));
...
program.hook("preAction", (_parent, action) => {
    if (UPKEEP_SKIP.has(action.name()))
        return;
...
program
    .command("build")
    .description("Build graft/ from your code — wiring graph + per-file cards ($0, no key). " +
    ...
    .option("--follow-submodules", "include initialized Git submodules recursively; persisted for later builds and automatic refreshes")
    .option("--no-follow-submodules", "exclude Git submodules; persisted for later builds and automatic refreshes (default)")
    .option("--follow-nested-repos", "include nested Git clones the index does not track (a multi-repo manifest checkout, or any repo cloned into the tree) " +
    "as ONE graph, so imports across them resolve; persisted for later builds and automatic refreshes")
    .option("--no-follow-nested-repos", "exclude untracked nested Git clones; persisted for later builds and automatic refreshes (default)")
    .option("--include-dir <name>", "override SKIP_DIRS for this repo's walks — repeatable (e.g. --include-dir build --include-dir tools); " +
    "persisted, so a later build (and the hooks/refresh path) include it without the flag; dot-dirs are never overridable", (val, prev) => [...prev, val], [])
    .option("--only-dir <path>", "only index files under this repo-relative path — repeatable; the wiring walk and the --deep " +
    "concept pass both honor it. Recorded in the graph fingerprint so a later build " +
    "(and the hooks/refresh path) walks the same set; everything outside the list is skipped", (val, prev) => [...prev, val], [])
    ...
    if (deep && !resolved.apiKey) {
        deep = false;
        console.error("⚠ no API key set — falling back to the structural build (no LLM summaries).\n" +
    ...
    if (deep && resolved.usedLegacyEnv) {
        console.error("⚠ using OPENROUTER_API_KEY (deprecated) — prefer GRAFT_API_KEY + GRAFT_BASE_URL.");
    }
    // Workspace parent: build each child into its OWN graft/ + a workspace index.
    const buildRoot = resolve(dir);
    const buildGlobalDir = program.opts().dir;
    ...
    if (deep) {
        const c = await engine.init(dir, {
            extensions: opts.extensions,
            onlyDirs,
            onProgress: ({ phase, index, total, file }) => process.stderr.write(`\r${phase === "summarize" ? "reading" : "writing"} concepts ${index + 1}/${total}: ${file.slice(0, 40).padEnd(40)}`),
        }).catch((err) => {
        ...
        console.log(`✓ concepts: ${c.nodes} nodes, ${c.links} links from ${c.files} files (${c.summarized} read, ${c.cached} cached)`);
    ...
    const g = await engine.graph(dir, {
        ...
        lsp: opts.lsp,
        onlyDirs,
        onProgress: ({ phase, index, total, file }) => process.stderr.write(`\r${phase === "enrich" ? "summarizing" : "parsing"} ${index + 1}/${total}: ${file.slice(0, 50).padEnd(50)}`),
    }).catch((err) => {
    ...
    if (g.seededFrom)
        console.log(`  seeded: copied a starting graph from ${g.seededFrom} (git worktree)`);
    if (deep) {
        const m = g.meaning;
    ...
    if (deep) {
        const m = g.meaning;
        const failed = m.failedFiles > 0 || m.fatal !== undefined || conceptErrors.length > 0 || conceptFatal !== undefined;
        if (failed) {
            ...
            if (conceptFatal)
                console.error(`  concepts: ${conceptFatal}`);
            if (m.fatal)
                console.error(`  summaries: ${m.fatal}`);
            if (m.failedFiles > 0) {
            ...
            if (conceptErrors.length > 0)
                console.error(`  ${conceptErrors.length} concept-pass error(s).`);
            console.error(`  meaning coverage: ${ready}/${total} symbols (${pct}%).`);
            console.error("  Nothing computed was lost: re-run `graft build --deep` to resume from what is cached.\n" +
...
program
    ...
    .action(async (query, dirArg, opts) => {
    const dir = noteQuery(queryRoot(dirArg));
    await refreshBefore(dir, opts);
    const askGlobalDir = program.opts().dir;
...
program
    ...
    if (opts.json)
        ...
        process.stdout.write(formatSkeleton(r));
});
program
    .command("check")
    ...
    warnUnsupportedExtensions(opts.extensions);
    const dir = noteQuery(queryRoot(dirArg));
    const checkGlobalDir = program.opts().dir;
    ...
    if (opts.json) {
        ...
        if (r.missing) {
            console.log("deep layer: not built (run `graft build --deep` for concept nodes) — wiring graph is the source of truth");
        }
        else {
    ...
    if (bothMissing || markdownFail || wiringFail)
        process.exit(1);
});
program
    .command("stats")
...
program
    ...
    if (opts.export) {
        ...
        console.log(`graft viz → ${out.file} (${kb} kB, ${out.contextNodes} concept nodes, ${out.codeNodes} code nodes)`);
        return;
    }
...
program
    .command("callers")
    .description("Who calls/references a symbol ($0, no LLM). --direction out gives callees (what it calls); --depth N (or all) walks transitively for full blast radius")
    ...
    .option(...NO_REFRESH_FLAG)
    .action(async (symbol, dirArg, opts) => {
    const dir = noteQuery(queryRoot(dirArg));
    await refreshBefore(dir, opts);
    const globalOpts = program.opts();
...
program
    .command("blast")
    .description("Blast radius of a diff: what depends on the lines this change touched ($0, no LLM). " +
...
program
    ...
    .option("--json", "output as JSON")
    .option(...NO_REFRESH_FLAG)
    .action(async (pattern, dirArg, opts) => {
    const dir = noteQuery(queryRoot(dirArg));
    await refreshBefore(dir, opts);
    const globalOpts = program.opts();
...
program
    .command("map")
    .description("Token-budgeted repo orientation — directory clusters, per-directory hubs, and global hotspots from the wiring graph ($0, no LLM)")
    .argument(...DIR_ARG)
...
program
    ...
    if (opts.listAgents) {
        for (const id of [...hostIds(), "claude"])
            console.log(id);
        return;
    }
    ...
    const noAgents = opts.agents === false;
    let ids;
    ...
    const children = isWorkspaceBuildRoot(repo, program.opts().dir)
    ...
    if (brainLink) {
        ...
        if (res.warning)
            console.error(`⚠ brain: ${res.warning}`);
        else
            console.error(`✓ brain: pulled ${res.ruleCount} rule(s) from ${brainLink.brainId}`);
        for (const w of res.writes)
            console.error(`✓ brain rules: ${w.path} (${w.action})`);
    ...
    track("init_completed", { agents: [...ids].sort().join(","), consent: consent === undefined ? "unasked" : String(consent) }, { repo });
});
/** One repo's worth of `init` writes — the parent, then each workspace child. */
function wireTarget(repo, ids, ctx) {
    ...
    const retracted = changed(runRetract(repo, { home, apply: true, global: opts.global, cache: false, exclude: ids })).filter((r) => r.action !== "skipped-unparseable");
    for (const r of retracted)
        console.error(`- removed ${r.path} (${r.what}) — agent not selected`);
    if (wantClaude) {
        ...
        for (const s of res.shims)
            console.error(`✓ wrote ${s}`);
        ...
        if (!wantStatusline)
            console.error("· skipped Claude Code statusLine (--no-statusline)");
    ...
    if (others.length > 0) {
        ...
        for (const w of r.written)
            console.error(`✓ ${w.id}: ${w.path} (${w.action})`);
        for (const m of r.mcp)
            console.error(`✓ mcp ${m.id}: ${m.path} (${m.action})`);
        for (const h of r.hooks)
            console.error(`✓ hook ${h.id}: ${h.path} (${h.action})`);
...
function formatRetractions(rs, apply) {
    ...
    for (const [host, items] of byHost) {
        ...
        for (const r of items) {
            const mark = r.action === "skipped-unparseable"
                ? "⚠"
                : r.action === "deleted"
                    ? "-"
                    : "~";
            const note = r.action === "skipped-unparseable"
...
brain
    ...
    const graph = loadGraphCached(contextDirFor(repo, program.opts().dir));
    if (!graph)
        console.error("· no graph yet — run `graft build` first so rules can be anchored to symbols");
    ...
    if (built.warning)
        console.error(`⚠ ${built.warning}`);
    console.error(`· ${d.commits.length} commits, ${d.threads.length} discussions, ${d.symbols.length} symbols, ${d.sources.length} stated sources`);
...
brain
    ...
    // the team decided.
    const graph = loadGraphCached(contextDirFor(repo, program.opts().dir));
    const applied = rulesForPointers((graph?.nodes ?? []).map((n) => `${n.path}:${n.span}`), rules, graph);
    if (opts.json) {
